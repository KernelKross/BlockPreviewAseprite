...
function init(plugin)
	...
	onenabled = function()
		return app.activeCel ~= nil and app.activeSprite.width < maxSize+1 and app.activeSprite.height < maxSize+1
	end,
	onclick = function()
		...
		:canvas{
			...
			height=256,
			onpaint=function(ev)
				if app.activeSprite and app.activeSprite.width < maxSize+1 and app.activeSprite.height < maxSize+1 then
			...
			onchange = function()
				...
				if timer then
					if spin == 0 then
						...
						timer.interval = 1 / blockPreviewDialog.data.fps
					end
				end
				blockPreviewDialog:repaint()
			end
		...
		:slider{
			...
			value = 30,
			onchange = function()
				if timer and (blockPreviewDialog.data.spin or 0) ~= 0 then
					timer.interval = 1 / blockPreviewDialog.data.fps
				else
					timer.interval = 0
				end
		...
		local function onSiteChange()
			blockPreviewDialog:repaint()
			if not blockPreview or not blockPreviewDialog or not app.activeSprite then
				lastBounds = blockPreviewDialog.bounds
				blockPreviewDialog.bounds = Rectangle(-1000, -1000, 1, 1)
				return
			end
			if lastBounds then
				blockPreviewDialog.bounds = lastBounds
			end
			if not app.activeSprite.properties.kk_eventRegistered then
				app.activeSprite.events:on('change', onChange) -- app.activeSprite is current active sprite, isn't dynamic, so I have to add it to ever new sprite
				app.activeSprite.events:on('layervisibility', onChange)
				app.activeSprite.properties.kk_eventRegistered = true -- Custom variable to make a check for each active sprite and enable it's "change" event
			end
		end
		app.events:on('sitechange', onSiteChange)
		-- Missing cleanup? not for sitechange, but for site close or w/e
		onSiteChange() -- run when Block Preview is activated
	end
}
end
...
timer = Timer{
	interval = 0,
	ontick = function()
		if not blockPreview or not blockPreviewDialog or not app.activeSprite then return end
		local val = (blockPreviewDialog.data and blockPreviewDialog.data.spin) or 0
		if val ~= 0 then
